...
export default function Register({ register_login }) {
  ...
  const handleSubmit = (event) => {
    ...
    const register_hashedPassword = SHA256(password).toString();
    ...
    if (account && email && password) {
      axios
        .post('/japi/stu13/quasar2/api/abc/register', {
          name: account,
          email: email,
          password: register_hashedPassword,
        })
        .then((response) => {
          if (response.status === 200) {
            sessionStorage.setItem("register_account", account)
            sessionStorage.setItem("register_password", register_hashedPassword)
            register_login()
